...
## 리포트 예시 — 자동검사는 "통과", 수동검사가 3건 적발

`@verify-alf-code-node`에 아래 Code node JS를 넘기면 실제로 이런 리포트가 나온다. (전체 실행 로그: `tests/results.md`)
...
**자동 스크립트만 돌렸다면 이 3건(R3·R5·R6)은 그대로 배포됐을 것이다.** 이게 2단계 구조를 쓰는 이유다.
...
**예선 결과:** 채널코퍼레이션 "AX 인재전쟁" 예선에 제출 → 참가작 5,300여 건 중 **상위 약 10%** 성적으로 "AI 인재"에 선정. 다만 그 등급으로는 **본선(60선) 진출 컷에는 못 미쳤다.** "상위 10%"와 "본선 탈락"은 모순이 아니라 같은 예선 트랙 안에서 등급만 갈린 것이다.

**무엇이 부족했나 (예선 채점 축 기준):** 예선은 6개 축으로 채점됐다 — ①실제 구현 여부 ②확인 가능한 출처 ③문제(병목) 좁히기 ④정직성 ⑤AI 역할 분담 ⑥제3자 재현 증거. 이 도구는 ①구현·④정직성(이 README의 한계 섹션과 "확인 불가"·"해당 규칙 대상 아님" 상태 구분)·⑤결정론 설계에선 강점이었지만, 정작 **본선을 가른 최대 축인 ②에서 무너졌다.**

- **핵심(축2): 문제를 "관측"한 게 아니라 문서에서 "연역"했다.** 우리 근거(공식 문서)는 "이런 제약이 **존재한다**"까지만 증명한다. "이 위반이 실무에서 **자주·비싸게 난다**"는 건 포럼·이슈·장애 사례 같은 제3자 증거로 하나도 입증하지 못했다. 실재가 확인된 고통이 아니라, 문서에서 추론한 가설을 잘 푼 꼴이었다.
- **차별화 약점 (왜 굳이 AI인가):** 검사 항목 상당수(`fetch`·금지 함수 탐지 등)는 전용 린터/파서로도 대체 가능해서 AI를 써야 할 이유가 약했다. R6(인자 순서)·분기별 `save` 누락 같은 문맥 판단이 AI의 값이었지만, 그 값을 "자주 일어나는 문제"로 뒷받침하지 못하니 설득력이 떨어졌다.
- **부차 실점:** 최종 판정을 런타임 AI(2단계 수동)에 남겨 재현성이 흔들릴 수 있었고(축5), 플러그인 설치 의존은 심사 환경 재현 리스크였다(축6).

**다음에 바꿀 것:** 문제 후보를 고를 때 "문서에 제약이 있다"가 아니라 "현장에서 이 고통이 자주 관측된다"를 **제3자 증거 링크로 먼저 입증**하고, 데모엔 클린 환경(새 폴더 clone→install→run) 재현 로그를 붙이고, 최종 판정·계산은 결정론 코드로 고정한다.
